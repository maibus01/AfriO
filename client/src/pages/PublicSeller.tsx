...
export default function PublicSeller() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        axios
          .get(`${API}/styles/business/${businessId}`)
  ...
  if (!business)
    return <div className="p-20 text-center font-bold">Seller not found</div>;
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-20">
      ...
      <div className="fixed top-0 left-0 right-0 p-4 flex justify-between items-center z-50 pointer-events-none">
        <button
          onClick={() => navigate(-1)}
          className="p-3 bg-white/20 backdrop-blur-xl text-white rounded-full pointer-events-auto shadow-lg border border-white/20"
        >
          <ArrowLeft size={22} />
        </button>
