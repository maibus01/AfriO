...
function ColorModalManager({ savedData, onClose, onSave }: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/50 backdrop-blur-md p-4">
      <div className="bg-white dark:bg-zinc-900 w-full max-w-md p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-xl">
        <h3 className="text-sm font-bold text-zinc-900 dark:text-zinc-50 mb-2">Configure Colors</h3>
        <input 
          type="text" 
          value={input} 
        ...
        <div className="flex gap-2 justify-end">
          ...
          <button 
            onClick={() => onSave(input.split(",").map(c => c.trim()).filter(Boolean))} 
...
export default ColorModalManager;