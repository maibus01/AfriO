    ...
    <main className="min-h-screen bg-slate-50/50 pb-32">
      ...
      <nav className="fixed top-0 left-0 right-0 p-4 flex justify-between items-center z-50 pointer-events-none">
        ...
        <div className="flex gap-2 pointer-events-auto">
          <button className="p-3 bg-white/80 backdrop-blur-md text-slate-900 rounded-full shadow-xl active:scale-90 transition-transform">
            <Share2 size={20} />
          </button>
          <button className="p-3 bg-white/80 backdrop-blur-md text-slate-900 rounded-full shadow-xl active:scale-90 transition-transform text-red-500">
            <Heart size={20} />
          </button>
        </div>
      </nav>
          ...
          <div className="flex-1 px-5 md:px-0">
            <div className="bg-white md:border border-slate-100 p-8 md:rounded-[3rem] md:shadow-xl md:shadow-slate-200/50">
              <div className="flex flex-col mb-6">
                <span className="text-[10px] font-black text-orange-600 uppercase tracking-[0.3em] mb-2">
                  New Collection
                </span>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="bg-green-50 text-green-600 text-[10px] font-black px-2 py-1 rounded-md uppercase">
                    In Stock
                  </span>
                </div>
              </div>
              ...
              <div
                onClick={() =>
                  navigate(`/business/${product.businessId?._id}/public`)
                }
              ...
              <div className="space-y-6 mb-10">
                ...
                <div className="flex items-center justify-between p-2 bg-slate-50 rounded-2xl">
                  <span className="ml-4 font-black text-[10px] uppercase text-slate-400">
                    Order Quantity
                  </span>
                  <div className="flex items-center gap-4">
                    <button
                      onClick={() => setQuantity(Math.max(1, quantity - 1))}
                      className="w-10 h-10 bg-white rounded-xl shadow-sm flex items-center justify-center text-slate-900 active:scale-90 transition-transform"
                    >
                      <Minus size={18} />
                    </button>
                    ...
                    <button
                      ...
                      className="w-10 h-10 bg-slate-900 rounded-xl shadow-sm flex items-center justify-center text-white active:scale-90 transition-transform"
                    >
                      <Plus size={18} />
                    </button>
                  </div>
      ...
      <div className="max-w-7xl mx-auto px-5 mt-20">
        <div className="flex items-center justify-between mb-8">
          ...
          <button className="text-[10px] font-black uppercase text-slate-400 border-b-2 border-slate-100 pb-1">
            View All
          </button>
        </div>
          ...
          {styles.map((s) => (
            ...
            >
              ...
              <div className="absolute bottom-6 left-6 right-6">
                <p className="text-white text-sm font-black leading-tight tracking-tight uppercase">
                  {s.title}
                </p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
          {products.slice(0, 10).map((p) => (
            ...
            >
              ...
              <div className="p-4">
                <p className="font-black text-[10px] text-slate-400 uppercase tracking-widest mb-1 truncate">
                  {p.name}
                </p>
