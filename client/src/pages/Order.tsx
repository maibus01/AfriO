...
import {
  ...
  ShieldCheck, CreditCard, Loader2, CheckCircle2
} from "lucide-react";
...
const OrderPage = () => {
  ...
  // =========================
  // RENDER INTERNAL SUCCESS PAGE
  // =========================
  if (orderSuccess) {
    return (
      <div className="h-screen bg-white flex flex-col items-center justify-center px-6 text-center">
        <div className="w-20 h-20 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center mb-6 shadow-xl shadow-emerald-50">
          <CheckCircle2 size={42} className="stroke-[2.5]" />
        </div>
        <h1 className="text-3xl font-black text-slate-900 tracking-tighter mb-2">Order Submitted!</h1>
        <p className="text-sm text-slate-500 max-w-xs mb-8 font-medium leading-relaxed">
          Your order <span className="font-black text-slate-800">#{finalOrderId.slice(-6).toUpperCase()}</span> has been securely logged. We are verifying your bank transfer now.
        </p>
        <div className="w-full max-w-sm space-y-3">
          <button 
            onClick={() => navigate("/orders-hub")} 
            className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black transition-all hover:bg-slate-800"
          >
            Track My Orders
          </button>
          <button 
            onClick={() => navigate("/")} 
            className="w-full bg-slate-50 text-slate-600 py-4 rounded-2xl font-bold transition-all hover:bg-slate-100"
          >
            Continue Shopping
          </button>
        </div>
      ...
      <div className="max-w-xl mx-auto p-5 space-y-5">
        ...
        <div className="p-4 bg-orange-50 rounded-2xl flex gap-3 items-start">
          ...
          <p className="text-xs text-orange-800 leading-relaxed">
            <b>Secure Payment Instruction:</b> Please transfer the exact amount above to the designated account. Once you complete the transfer, click confirm below to lock in your order.
          </p>
        </div>
      </div>

      {/* FIXED FOOTER BUTTON - FLOATING ABOVE NAVBAR */}
      <div className="fixed bottom-16 left-0 right-0 p-4 bg-white/95 backdrop-blur-md border-t border-slate-100 z-40 flex justify-center">
        <button
          onClick={handleCompleteOrder}
          disabled={processingOrder || !selectedAccount}
          className="max-w-xl w-full bg-emerald-600 hover:bg-emerald-700 text-white py-4 rounded-2xl font-black flex items-center justify-center gap-3 transition-all active:scale-[0.98] disabled:bg-slate-300 shadow-lg shadow-emerald-100"
        >
          {processingOrder ? (
