...
export default function StylesPage() {
  const [activeTab, setActiveTab] = useState<"portfolio" | "requests">(
    "portfolio",
  );
    ...
    const fetchInitialData = async () => {
      try {
        const [bizRes, styleRes] = await Promise.all([
          ...
          axios.get(`${API}/styles/me`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);

        const tailor = bizRes.data.data.find(
          (b: any) => b.category === "tailor",
        );
  ...
  const updateStyle = async () => {
    ...
    try {
      ...
      setStyles((prev) =>
        prev.map((s) => (s._id === editMode._id ? res.data.data : s)),
      );
      setEditMode(null);
    } catch (err) {
      console.log(err);
          ...
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="p-8 border-b flex justify-between items-center">
              <h2 className="text-2xl font-black text-slate-900">
                {editMode ? "Refine Style" : "New Creation"}
              </h2>
              <button
                onClick={() => {
                  setOpen(false);
            ...
            <div className="p-8 space-y-5">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    editMode
                      ? setEditMode({ ...editMode, image: e.target.value })
