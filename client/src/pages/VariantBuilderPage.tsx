...
//   basePrice: number;
//   onClose: () => void;
//   // Add this line below to fix the type checking error:
//   onSave: (generatedVariants: any, attributeFiles: any) => void; 
//   onSuccess?: () => void; // Optional if still used internally
// }
...
//     const file = attributeFiles[`${key}:${value}`];
...
//         // 📸 Match files for this variant's active options selection
//         const files = buildVariantFiles(variant, attributeFiles);

//         files.forEach((file) => {
//           formData.append("images", file); // Must match backend multer target: upload.array("images")
//         });
...
//   // 💾 FIRES INTERNAL FROM COMPONENT
//   const handleLocalSave = async () => {
//     if (variants.length === 0) return;
//     setIsSaving(true);
//     setErrorMessage(null);
//     try {
//       await saveVariantsToDB(productId, variants, attributeFiles);
//       if (onSuccess) onSuccess();
//     } catch (err: any) {
//       setErrorMessage(err?.message || "Failed saving compilation sequence to DB.");
//     } finally {
//       setIsSaving(false);
//     }
//   };
...
//                         const SkinnerBoxImg = !!attributeImages[compositeKey];
//                         return (
...
//                       className="w-full border border-gray-200 px-3 py-2 md:px-2.5 md:py-1.5 rounded-md text-xs font-mono uppercase focus:ring-1 focus:ring-zinc-900 focus:outline-none disabled:opacity-60"
//                     />
//                   </div>
...
//                         className="w-full border border-gray-200 pl-6 pr-2.5 py-2 md:py-1.5 rounded-md text-xs focus:ring-1 focus:ring-zinc-900 focus:outline-none disabled:opacity-60"
//                       />
//                     </div>
//                   </div>
...
//                         disabled={isSaving}
//                         className="p-2 md:p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-md transition-colors border border-gray-100 md:border-none flex items-center justify-center bg-gray-50 md:bg-transparent disabled:opacity-30"
...
export default function VariantBuilderPage({
  ...
  // 💾 FIRES INTERNAL FROM COMPONENT - DELEGATES UPWARD VIA PROPS
  const handleLocalSave = async () => {
    if (variants.length === 0) return;
    setIsSaving(true);
    setErrorMessage(null);
    try {
      // 🚀 Safely passes state payloads back to Parent Context
      await onSave(variants, attributeFiles);
      if (onSuccess) onSuccess();
    } catch (err: any) {
